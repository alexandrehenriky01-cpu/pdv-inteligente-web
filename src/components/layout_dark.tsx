...
const SectionHeader = ({ id, title, icon: Icon, isAi = false, isOpen, collapsed, onToggle }: SectionHeaderProps) => {
  if (collapsed) {
    return (
      <button onClick={() => onToggle(id)} title={title} className={`mt-2 flex w-full items-center justify-center rounded-xl border p-3 transition-all duration-200 backdrop-blur-md ${isOpen ? (isAi ? 'bg-violet-500/12 border-violet-500/20 text-violet-300' : 'bg-white/[0.07] border-white/10 text-white') : (isAi ? 'text-violet-400 hover:bg-violet-500/12 border-transparent hover:border-violet-500/15' : 'text-slate-400 hover:bg-white/[0.06] border-transparent hover:border-white/10')}`}>
  ...
  return (
    <button onClick={() => onToggle(id)} title={title} className={`mt-2 flex w-full items-center justify-between overflow-hidden rounded-xl border p-3 transition-all duration-200 backdrop-blur-md ${isOpen ? (isAi ? 'bg-violet-500/12 border-violet-500/20 text-violet-300' : 'bg-white/[0.07] border-white/10 text-white') : (isAi ? 'text-violet-400 hover:bg-violet-500/10 border-transparent hover:border-violet-500/15' : 'text-slate-300 hover:bg-white/5 border-transparent hover:border-white/10')}`}>
...
export function Layout({ children }: LayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-[#060816] text-slate-100 overflow-hidden">
      <aside className={`${sidebarCollapsed ? 'w-24' : 'w-72'} relative z-10 flex shrink-0 flex-col border-r border-white/10 bg-[#050913]/95 shadow-[0_25px_60px_rgba(0,0,0,0.45)] backdrop-blur-xl transition-all duration-300`}>
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,_rgba(139,92,246,0.18),_transparent_28%),linear-gradient(180deg,_rgba(255,255,255,0.02),_transparent_30%)]" />
        ...
        <div className={`relative z-10 ${sidebarCollapsed ? 'px-3 py-5' : 'px-5 py-5'} border-b border-white/10 bg-[#08101f]/30 flex items-center ${sidebarCollapsed ? 'justify-center' : 'justify-between gap-3'}`}>
          ...
          {!sidebarCollapsed && (
            <button onClick={() => setSidebarCollapsed(true)} className="rounded-xl border border-transparent p-2 text-slate-400 transition-all hover:border-white/10 hover:bg-white/[0.06] hover:text-white" title="Recolher menu">
        ...
        {sidebarCollapsed && (
          <div className="relative z-10 px-3 pt-3">
            <button onClick={() => setSidebarCollapsed(false)} className="flex w-full items-center justify-center rounded-xl border border-transparent p-2.5 text-slate-400 transition-all hover:border-white/10 hover:bg-white/[0.06] hover:text-white" title="Expandir menu">
          ...
          {temPermissao('ia') && (
            <>
              ...
              {menusAbertos.centralAurya && (
                <div className={`space-y-1 mt-1 mb-4 animate-in slide-in-from-top-2 fade-in duration-200 rounded-2xl border border-violet-500/15 bg-violet-500/[0.06] backdrop-blur-md ${sidebarCollapsed ? 'p-1.5' : 'p-2'}`}>
                ...
                <div className={`space-y-1 mt-1 mb-2 ${sidebarCollapsed ? 'p-1' : 'pl-1'} animate-in slide-in-from-top-2 fade-in duration-200`}>
                  ...
                  {isGestor && (
                    <>
                      ...
                      <MenuItem to="/configuracoes-loja" icon={Settings} label="Minha Loja" isActive={location.pathname === '/configuracoes-loja'} collapsed={sidebarCollapsed} />
                    </>
                  )}
                </div>
              )}
            ...
            <>
              ...
              {menusAbertos.comprasSuprimentos && (
                <div className={`space-y-1 mt-1 mb-2 ${sidebarCollapsed ? 'p-1' : 'pl-1'} animate-in slide-in-from-top-2 fade-in duration-200`}>
                  ...
                  <MenuItem to="/compras/InteligenciaComprasService" icon={BrainCircuit} label="Análise Aurya" isAi={true} isActive={location.pathname === '/compras/InteligenciaComprasService'} collapsed={sidebarCollapsed} />
                </div>
              )}
            </>
          )}
            ...
            <>
              ...
              {menusAbertos.financeiroInteligente && (
                <div className={`space-y-1 mt-1 mb-2 ${sidebarCollapsed ? 'p-1' : 'pl-1'} animate-in slide-in-from-top-2 fade-in duration-200`}>
                  ...
                  <MenuItem to="/financeiro/gestao-titulos" icon={CheckSquare} label="Títulos (Pagar/Receber)" isActive={location.pathname === '/financeiro/gestao-titulos'} collapsed={sidebarCollapsed} />
                  <MenuItem to="/financeiro/caixas" icon={Landmark} label="Contas e Caixas" isActive={location.pathname === '/financeiro/caixas'} collapsed={sidebarCollapsed} />
                  <MenuItem to="/financeiro/extrato" icon={ListOrdered} label="Extrato de Contas" isActive={location.pathname === '/financeiro/extrato'} collapsed={sidebarCollapsed} />
            ...
            <>
              ...
              {menusAbertos.contabilidadeAnalise && (
                <div className={`space-y-1 mt-1 mb-2 ${sidebarCollapsed ? 'p-1' : 'pl-1'} animate-in slide-in-from-top-2 fade-in duration-200`}>
                  ...
                  <MenuItem to="/contabil/plano-contas" icon={Network} label="Plano de Contas" isActive={location.pathname === '/contabil/plano-contas'} collapsed={sidebarCollapsed} />
                  <MenuItem to="/contabil/extrato" icon={FileText} label="Livro Razão" isActive={location.pathname === '/contabil/extrato'} collapsed={sidebarCollapsed} />
                  ...
                  <MenuItem to="/contabil/dashboardglobal" icon={CheckSquare} label="Fechamento Contábil" isActive={location.pathname === '/contabil/fechamento'} collapsed={sidebarCollapsed} />
                </div>
              )}
            </>
          )}
        ...
        <div className="relative z-10 shrink-0 border-t border-white/10 bg-[#08101f]/40 p-4 backdrop-blur-xl">
          {!sidebarCollapsed ? (
            <>
              <div className="mb-4 flex items-center gap-3 overflow-hidden rounded-2xl border border-white/10 bg-white/[0.06] px-3 py-3 backdrop-blur-md">
              ...
              <button
                ...
                className="flex w-full items-center justify-center gap-2 rounded-xl border border-white/10 bg-white/[0.06] py-2.5 text-sm font-bold text-slate-300 backdrop-blur-md transition-all hover:border-red-500/20 hover:bg-red-500/10 hover:text-red-300"
              >
      ...
      <main className="flex-1 min-w-0 overflow-y-auto overflow-x-hidden bg-[radial-gradient(circle_at_top,_rgba(139,92,246,0.18),_transparent_30%),linear-gradient(180deg,_#0b1020_0%,_#070b17_100%)] p-6 md:p-8">
        {children}
      </main>
    </div>
  );
