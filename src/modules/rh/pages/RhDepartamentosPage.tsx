...
import {
  ...
  listDepartamentos,
  updateDepartamento,
  type DepartamentoFormInput,
} from '../services/rhApi';
import type { RhDepartamentoView } from '../types/rh.types';
...
export default function RhDepartamentosPage(): JSX.Element {
  const [items, setItems] = useState<RhDepartamentoView[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<RhDepartamentoView | null>(null);
  const [form, setForm] = useState<DepartamentoFormInput>({ codigo: '', nome: '', descricao: null });
  ...
  const reload = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await listDepartamentos({ pageSize: 200 });
      setItems([...res.items]);
    } catch (err) {
  ...
  const open = (target: RhDepartamentoView | null): void => {
    setEditing(target);
    setForm(target
      ? { codigo: target.codigo, nome: target.nome, descricao: target.descricao, ativo: target.ativo }
  ...
  const submit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    setError(null);
    try {
      if (editing) await updateDepartamento(editing.id, form);
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={4} className="px-4 py-8 text-center text-slate-400">Nenhum departamento cadastrado.</td></tr>
            ) : (
              items.map((d) => (
                <tr key={d.id} className="border-t border-white/5 hover:bg-white/5">
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded-md border px-2 py-0.5 text-xs ${d.ativo ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-300' : 'border-slate-500/30 bg-slate-500/10 text-slate-300'}`}>
                      {d.ativo ? 'Ativo' : 'Inativo'}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="inline-flex gap-1">
                      <button type="button" onClick={() => open(d)} className="rounded-md border border-white/10 bg-white/5 px-2 py-1 text-xs text-slate-200 hover:bg-white/10">Editar</button>
                      <button type="button" onClick={() => void remove(d.id)} className="rounded-md border border-rose-500/30 bg-rose-500/10 px-2 py-1 text-xs text-rose-200 hover:bg-rose-500/20">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <form onSubmit={(e) => void submit(e)} className="w-full max-w-md space-y-3 rounded-2xl border border-white/10 bg-slate-950 p-6 shadow-2xl">
            <div className="flex items-center justify-between">
              <h2 className="text-lg font-semibold text-white">{editing ? 'Editar departamento' : 'Novo departamento'}</h2>
            ...
            <label className="block text-xs uppercase tracking-wide text-slate-400">
              <span>Descrição</span>
              <textarea rows={3} value={form.descricao ?? ''} onChange={(e) => setForm({ ...form, descricao: e.target.value || null })} className={`mt-1 ${inputCls}`} />
            </label>
