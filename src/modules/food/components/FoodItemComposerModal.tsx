...
export function FoodItemComposerModal({
  ...
  const panelClass = isSheet
    ? `relative z-10 flex max-h-[92vh] w-full ${sheetMaxWidthClass} flex-col rounded-t-3xl border border-white/10 border-b-0 bg-[#060816] shadow-[0_-20px_60px_rgba(0,0,0,0.45)]`
      ...
      <div className={panelClass}>
        ...
        <div className="relative z-10 min-h-0 flex-1 overflow-y-auto overscroll-contain">
          <div
            className={
              isSheet
                ? 'relative mx-auto max-h-36 w-full overflow-hidden sm:max-h-40'
          ...
          <div className={`mx-auto max-w-3xl space-y-8 px-4 pt-6 ${isSheet ? 'pb-32' : 'pb-36'}`}>
                  ...
                  {tamanhosAtivos.map((t) => (
                    <li key={t.id}>
                      <button
                        ...
                        disabled={Boolean(tamanhoBloqueadoPizzaMulti && tamanhoBloqueadoPizzaMulti !== t.id)}
                        className={`flex w-full items-center justify-between rounded-2xl border px-4 py-3 text-left transition ${
                          tamanhoId === t.id
                            ? 'border-violet-400/60 bg-violet-500/20 text-white'
                            : tamanhoBloqueadoPizzaMulti && tamanhoBloqueadoPizzaMulti !== t.id
                              ? 'border-white/10 bg-white/[0.03] text-white/35 opacity-55'
            ...
            {isPizzaMulti &&
              ...
              (!precisaTamanho || tamanhoId) &&
              saboresSelecionados.length > 0 && (
                <section className="rounded-2xl border border-violet-400/25 bg-violet-500/10 px-4 py-3">
                  <h3 className="mb-2 text-sm font-semibold uppercase tracking-wider text-violet-200/90">
                    Sabores nesta pizza
                  </h3>
                  ...
                  <ul className="flex flex-wrap gap-2">
                    {saboresSelecionados.map((sid) => {
                      ...
                      return (
                        <li
                          key={sid}
                          className="rounded-full border border-violet-400/35 bg-violet-500/15 px-3 py-1 text-xs font-medium text-violet-100"
                        >
                          {nome}
                        </li>
                      );
